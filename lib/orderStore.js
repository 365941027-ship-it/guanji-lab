...
export function recordOrder(payload) {
  ...
  const amountRaw = p.amount || p.total_fee || p.paid_amount || p['实付金额'] || p['金额'] ||
    (p.data && (p.data.amount || p.data.total_fee || p.data['实付金额'] || p.data['金额']));
...
/**
 * 列出所有已记账的订单，按时间倒序（站长工具用，展示最近补过哪些）。
...
export default { recordOrder, queryOrder, listOrders };
