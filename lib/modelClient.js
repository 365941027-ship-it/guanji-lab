...
export async function callModel({ provider = 'deepseek', messages = [], maxTokens = 2000, temperature = 0.8, reasoningEffort = 'low' } = {}) {
  ...
  async function once(tokens) {
    const upstream = await fetch(url, {
      ...
      body: JSON.stringify(cfg.build(messages, tokens, temperature, reasoningEffort))
    });
  ...
  const data = await once(maxTokens);
  const text = cfg.parse(data);
  if (text) return text.trim();

  // 空正文常见原因：推理型模型把 token 全用在了「思考」上（finish_reason=length）。
  // 此时用更大的额度重试一次，避免用户拿到空结果。
  const choice = (data && data.choices && data.choices[0]) || {};
  const spentAll = choice.finish_reason === 'length';
  const retryTokens = Math.min(Math.max(maxTokens * 2, 4000), 16000);
  if (spentAll && retryTokens > maxTokens) {
    const retry = await once(retryTokens);
    const retryText = cfg.parse(retry);
    if (retryText) return retryText.trim();
  }
